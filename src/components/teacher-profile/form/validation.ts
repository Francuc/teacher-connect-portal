...
export const validateForm = (formData: FormData, t: (key: string) => string) => {
  ...
  if (!formData.firstName.trim()) errors.push(t("firstName"));
  if (!formData.lastName.trim()) errors.push(t("lastName"));
  if (!formData.email.trim()) errors.push(t("email"));
  if (!formData.bio.trim()) errors.push(t("bio"));
  ...
  if (!formData.schoolLevels || formData.schoolLevels.length === 0) {
    errors.push(t("selectAtLeastOneLevel"));
  }

  // Validate teaching locations (at least one required)
  if (!formData.teachingLocations || formData.teachingLocations.length === 0) {
    ...
    formData.teachingLocations.forEach(location => {
      switch (location) {
        case "Teacher's Place":
          ...
          if (!formData.pricePerHour.teacherPlace || 
              parseFloat(formData.pricePerHour.teacherPlace) <= 0) {
            errors.push(t("validPriceRequiredForTeacherPlace"));
          }
          break;
        case "Student's Place":
          if (!formData.pricePerHour.studentPlace || 
              parseFloat(formData.pricePerHour.studentPlace) <= 0) {
          ...
          break;
        case "Online":
          if (!formData.pricePerHour.online || 
              parseFloat(formData.pricePerHour.online) <= 0) {
