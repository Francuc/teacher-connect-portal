...
export const useTeachersData = () => {
  return useQuery({
    queryKey: ['teachers'],
    queryFn: async () => {
      console.log('useTeachersData: Starting fetch...'); // Debug log
      ...
      try {
        ...
        if (error) {
          console.error('useTeachersData: Error fetching teachers:', error);
          throw error;
        }

        console.log('useTeachersData: Teachers fetched successfully:', {
        ...
        console.error('useTeachersData: Unexpected error:', error);
        throw error;
      }
    },
